...
public class ClientHandler {
    ...
    private String getClientsList(){
        ...
        Set<ClientHandler> clientHandlers = server.getClientHandlers();
        for (ClientHandler ch: clientHandlers)
              {listOfClients = listOfClients.concat(ch.record.toString());
        }
    ...
    public void sendDirectMessage(String message) {
        ...
        if (!result[1].isBlank()  && !result[2].isBlank()) {
            if (!server.sendDirectMessage(result[1].toString(), result[2].toString())){
                sendMessage("There is no such client!");
            }
